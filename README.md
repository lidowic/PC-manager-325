...
## Что изменено для Windows

- `agent` запускается на Windows и слушает `9021/tcp`
- основной shell для удаленных скриптов: `PowerShell`
- команды питания используют штатный `shutdown`
- клиентский конфиг ориентирован на Windows-хосты
- удаленный рабочий стол подключается по URL внешнего web-based desktop gateway
- список машин хранится в локальной `SQLite` базе клиента
- клиентская БД хранит метки, профили подключения, локальные настройки и шаблоны скриптов
